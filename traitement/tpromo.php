...
if (isset($_POST['selectedElements'])) {
  ...
  foreach ($mutipledeleteId as $element) {
      ...
      $sql = "DELETE FROM `promotionacademique` WHERE id = $id";

      // Exécutez la requête
      if ($conn->query($sql) === TRUE) {
          // Suppression réussie
          $message = "Promotion parcours supprimée avec succès.";
  ...
  header("Location: ../PromotionAca.php"); // Redirige vers la page souhaitée
  exit();
}



if (isset($_POST['iddpro'])) {
  $sql = "DELETE FROM `promotionacademique` WHERE id=$deleteId"; 
...
if ($conn->query($sql) === TRUE) {
 ...
 $message = "Promotion académique Supprimée avec succès.";
  $_SESSION['error_message'] = $message; // Enregistre $message dans la session
header("Location: ../PromotionAca.php"); // Redirige vers la page souhaitée
exit();
} else {
  ...
  $id = intval($_POST['idpro']);
  $periode = $_POST['periode'];
  $parcours = $_POST['parcours'];
  $nompromo = $_POST['nompromo'];
  $nombapt = $_POST['nombapt'];
  $ro = $_POST['ro'];
  $so = $_POST['so'];
 
// Vérification des valeurs non vides
if (!empty($nompromo) && !empty($nombapt) && !empty($ro) && !empty($so)) {
  // Toutes les valeurs sont présentes, procéder à l'insertion dans la base de données

    if($id === 0){

      // Requête d'insertion
$sql = "INSERT INTO `promotionacademique`
(`nompromo`,`nombapteme`,`rentréeofficielle`,`sortieofficielle`,`idperio`,`idparc`)VALUES('$nompromo','$nombapt','$ro','$so','$periode','$parcours')";
    }else{

      $sql = "UPDATE promotionacademique SET nompromo='$nompromo',`nombapteme`='$nombapt',`rentréeofficielle`='$ro',`sortieofficielle`= '$so',`idperio`='$periode',`idparc`='$parcours' WHERE id=$id";    }
...
if ($conn->query($sql) === TRUE) {
 ...
 $message = "Promotion académique enregistré avec succès.";
  $_SESSION['error_message'] = $message; // Enregistre $message dans la session
header("Location: ../PromotionAca.php"); // Redirige vers la page souhaitée
exit();
} else {
...
header("Location: ../PromotionAca.php");
exit();
} 
