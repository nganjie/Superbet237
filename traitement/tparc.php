...
if (isset($_POST['selectedElements'])) {
  ...
  foreach ($mutipledeleteId as $element) {
      ...
      $sql = "DELETE FROM parcoursacademique WHERE id = $id";
  ...
  header("Location: ../ParcoursAca.php"); // Redirige vers la page souhaitée
  exit();
}



if (isset($_POST['iddparc'])) {
  $sql = "DELETE FROM parcoursacademique WHERE id=$deleteId"; 
...
exit();
  ...
  $id = intval($_POST['idparc']);
  $nomparc = $_POST['nomparc'];
...
if (!empty($nomparc)) {
  // Toutes les valeurs sont présentes, procéder à l'insertion dans la base de données
    ...
    if($id === 0){
      ...
      // Requête d'insertion
$sql = "INSERT INTO parcoursacademique (nomparc) VALUES ('$nomparc')";
    }else{

      $sql = "UPDATE parcoursacademique SET nomparc='$nomparc' WHERE id=$id";    }
...
header("Location: ../ParcoursAca.php");
exit();
} 
